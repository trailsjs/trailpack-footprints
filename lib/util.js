...
module.exports = {
  ...
  getControllerFootprints (app) {
    ...
    return _.flatten(_.map(controllers, (controller, controllerName) => {
      const controllerMantissa = lib.Util.getControllerMantissa(controllerName)

      return _.map(controller.methods, handlerName => {
        return {
          method: _.get(app.config, 'footprints.controllers.method') || [ 'GET', 'POST' ],
          path: lib.Util.getHandlerPath(app, prefix, controllerMantissa, handlerName),
  ...
  getModelFootprints (app) {
    ...
    return _.filter(_.get(app.config, 'footprints.models.routes'), route => {
  ...
  getControllerMantissa (controllerName) {
    return controllerName.replace(/(\w+)Controller/, '$1').toLowerCase()
  },
  ...
  getHandlerPath (app, prefix, controllerMantissa, handlerName) {
    return app.packs.router.util.getRoutePath([
      '/', prefix, controllerMantissa, handlerName
    ])
  }
}
